     ...
     int restart_stat;
{
  ...
  print_memory_usage ("Before Beginning of Ionization Cycles (and Photon Generation)");
  ...
  Debug ("CCC - Allocated photmain for memory check with NPHOT set to %d\n", NPHOT);
  ...
  Debug ("CCC - Freed  photmain after memory check\n");
  /* End of the test */
...
/* BEGINNING OF CYCLE TO CALCULATE THE IONIZATION OF THE WIND */
  ...
  {                             /* This allows you to build up photons in bunches */
    ...
    photmain = p = (PhotPtr) calloc (sizeof (p_dummy), NPHOT + 1);
    photmain_allocated = TRUE;
    Debug ("CCC - Allocated photmain for wcycle %d with NPHOT of %d\n", geo.wcycle, NPHOT);

    if (geo.wcycle == cycle_start)
    {
      print_memory_usage ("Beginning of the first Ionization Cycles (after Photon Structure Allocated)");
    }
    ...
    free (photmain);
    photmain_allocated = FALSE;
    Debug ("CCC - freed photmain at end of of %d cycles\n", geo.wcycle);
...
/* Next steps to speed up extraction stage */
  ...
  photmain = p = (PhotPtr) calloc (sizeof (p_dummy), NPHOT + 1);
  photmain_allocated = TRUE;
  Debug ("CCC - Allocated photmain at beginning of detailed spectrum, with NPHOT %d\n", NPHOT);
     ...
     int nphot;
{
  ...
  Log_separator_line (TRUE);    /* adds ----- printout for legibility */
  ...
  for (nn = 0; nn < N_ISTAT; nn++)
  {
    Debug ("XXX stat %8d     %8d      %12.3e    %12.3e\n", nn, nphot_istat[nn], z_abs[nn], z_orig[nn]);
  }
  for (nn = 0; nn < 20; nn++)
  {
    Debug ("XXX rad %8d     %12.3e    %12.3e\n", nn, radiated[nn], radiated_orig[nn]);
  }
  Debug ("XXX  rad  abs_all  %12.3e    %12.3e\n", z_abs_all, z_abs_all_orig);
  Debug ("XXX  rad  else  l  %12.3e    %12.3e\n", z_else, z_else_orig);
