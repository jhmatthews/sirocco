...
print_memory_usage (const char *label)
{
  ...
  Log_separator_line (0);
  Log ("SYS: Memory Usage: %s\n", label);
  ...
  Log ("SYS: User CPU time: %ld.%06ld sec, Sys CPU time: %ld.%06ld sec\n",
       (long) usage.ru_utime.tv_sec, (long) usage.ru_utime.tv_usec, (long) usage.ru_stime.tv_sec, (long) usage.ru_stime.tv_usec);
  Log ("SYS: Page faults (no I/O): %ld (with I/O): %ld\n", usage.ru_minflt, usage.ru_majflt);
...
/**********************************************************/
/**
 *  @brief Add a printed line to logfile and screen
 *
 *  @param[in] int newlines whether to add newline before 
 *                 and after (TRUE OR FALSE)
 **********************************************************/
void
Log_separator_line (int newlines)
{
  if (newlines)
    Log ("\n");
  Log ("------------------------------------------------------------\n");
